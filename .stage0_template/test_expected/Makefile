...
SLUG := $(shell yq '.info.slug' $(PRODUCT_FILE))
ORG := $(shell yq '.organization.git_org' $(PRODUCT_FILE))
LOG_FILE ?= .make.log
SPECS_DIR := $(CURDIR)/Specifications
MERGE_IMAGE := ghcr.io/agile-learning-institute/stage0_runbook_merge:latest
ALL_SERVICES := $(shell yq '.architecture.domains[].name' $(ARCH_FILE))
...
schemas:
	...
	@yq '.data_dictionaries[].name' ./Specifications/catalog.yaml | \
...
launch-services:
	...
	for svc in $(SERVICES); do \
		[ -z "$$svc" ] && continue; \
		REPO_LINES=$$(yq '.architecture.domains[] | select(.name == "'$$svc'") | .repos[] | select(.type == "api" or .type == "spa") | (.name + "|" + .template + "|" + (.publish // ""))' $(ARCH_FILE) 2>/dev/null) || true; \
...
clean-clone-build:
	...
	for svc in $(SERVICES); do \
		[ -z "$$svc" ] && continue; \
		REPO_LINES=$$(yq '.architecture.domains[] | select(.name == "'$$svc'") | .repos[] | select(.type == "api" or .type == "spa") | (.name + "|" + (.publish // ""))' $(ARCH_FILE) 2>/dev/null) || true; \
		[ -z "$$REPO_LINES" ] && continue; \
		echo "--- Domain: $$svc ---"; \
