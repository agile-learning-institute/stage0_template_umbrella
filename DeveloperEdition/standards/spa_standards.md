...
- **URL bootstrap:** Call **`bootstrapAuthFromUrl()`** from `spa_utils` once before the router mounts. Hash `#access_token=...&expires_at=...&roles=...` seeds localStorage (Developer Edition `login.html` / IdP-style callback). Query `?clear_stored_auth=1` clears stored tokens when needed.
