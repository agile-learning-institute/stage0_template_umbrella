...
- **GITHUB_TOKEN** – Classic token with `repo`, `workflow`, `write:packages` (and `delete:packages`, `delete_repo` for delete commands). Store in `~/.{{info.slug}}/GITHUB_TOKEN`.
...
- **Docker** – Logged in to `{{org.docker_host}}` (e.g. via `make update` from repo root). **Docker Buildx** is used for multi-arch builds in CI; local single-arch builds do not require it.
